...
function printUsage(pkg, serviceName, address, service) {
  ...
  serviceNames.map((name, index) => {
    // HEYLISTEN! We are changing this part until that commit is finally merged 😅
    // https://github.com/grpc/grpc-node/pull/408/commits
    /*console.log('    %s (%s, callback) %s %s', name.green,
      service[name].requestType.name,
      'returns'.gray,
      service[name].responseType.name);*/
    console.log('    %s %s', index === serviceNames.length - 1 ? '└──' : '├──', name.green);
  });
